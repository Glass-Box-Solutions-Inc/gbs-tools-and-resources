...
const BASE_MIGRATION_SQL = MIGRATION_SQL.slice(
  0,
  MIGRATION_SQL.indexOf("-- GLY-345 v3 additive delta"),
);
...
function expectReferenceFiltersBeforeLimit(statement: string): void {
  const claimFilter = statement.indexOf(
    "AND NOT EXISTS (\n             SELECT 1 FROM reversal_claim",
  );
  const currentFilter = statement.indexOf(
...
describe("GLY-345 SQL protocol and expand-contract source oracles", () => {
  it("keeps the READ COMMITTED speculative-insertion anchor transaction verbatim", () => {
    const source = readFileSync(
      new URL(
        "../src/tokens/durable/azure/postgres-control-plane.ts",
        import.meta.url,
      ),
      "utf8",
    );
    const isolation = source.indexOf(
    ...
    expect(source.slice(Math.max(0, isolation - 300), prepared)).toContain(
      "speculative-insertion waiting",
    );
  ...
  it("keeps the expand migration nullable/NOT VALID, guarded first, idempotent, and free of requiredness", () => {
    const delta = MIGRATION_SQL.slice(
      MIGRATION_SQL.indexOf("-- GLY-345 v3 additive delta"),
    );
    ...
    for (const column of [
      ...
      expect(delta).not.toContain(
        `ADD COLUMN IF NOT EXISTS ${column} NOT NULL`,
      );
    }
  ...
  it("discovers anonymous state checks by definition and seeds legacy supersession from DB now", () => {
    ...
    const seed = delta.slice(
      delta.indexOf("WITH migration_clock AS"),
      delta.indexOf("-- Added constraints"),
    );
  ...
  it("contains fatal legacy class/expiry guards and the E1 NULL-safe matter selector", () => {
    ...
    expect(
      source.match(/COALESCE\(p\.retention_class, 'matter'\) = 'matter'/g) ??
        [],
    ).toHaveLength(3);
    ...
    const selfHeal = source.slice(
      source.indexOf("async #selfHealFlushedClaim"),
      source.indexOf("async #expireLockedPending"),
    );
    ...
    const flush = source.slice(
      source.indexOf("public async flushClaim"),
      source.indexOf("public expirePendingDetach"),
    );
    expect(flush).toContain("clock_timestamp()");
    expect(flush).not.toContain(
      "superseded_at_ms = input.nowEpochMilliseconds",
    );
  });
});

describe("PostgresControlPlane maintenance completion idempotency", () => {
  const handle = brand<PreparedWriteHandle>(
  ...
  it("uses the database clock instead of binding producer time into prepared lifecycle age", async () => {
    const fixture = scriptedControlPlane([
      ...
      {
        rowCount: 1,
        rows: [{ tenant_id: "tenant-a", retention_class: "matter" }],
      },
      { rowCount: 1, rows: [] },
    ]);
    ...
    expect(
      fixture.statements.some((statement) =>
        statement.includes("EXTRACT(EPOCH FROM clock_timestamp()) * 1000"),
      ),
    ).toBe(true);
    const anchor = fixture.statements.findIndex((statement) =>
      statement.includes("INSERT INTO reversal_prepared"),
    );
  ...
  it("accepts markQuarantined after another worker already reached quarantined", async () => {
    ...
    await expect(
      fixture.controlPlane.markQuarantined({
        preparedBlobId: handle,
  ...
  it("accepts completeStaleUploadReclaim after another worker already deleted the row", async () => {
    ...
    await expect(
      fixture.controlPlane.completeStaleUploadReclaim(handle),
    ).resolves.toBeUndefined();
    expect(fixture.statements).toEqual([
      "BEGIN",
  ...
  it("accepts completeHardDeleteQuarantined after another worker already deleted the row", async () => {
    ...
    await expect(
      fixture.controlPlane.completeHardDeleteQuarantined(handle),
    ).resolves.toBeUndefined();
    expect(fixture.statements).toEqual([
      "BEGIN",
      expect.stringContaining("DELETE FROM reversal_prepared"),
  ...
  it.each([
    {
      ...
      complete: (controlPlane: PostgresControlPlane) =>
        controlPlane.markQuarantined({
          preparedBlobId: handle,
          quarantinedAtEpochMs: T0,
        }),
    },
    {
      ...
      complete: (controlPlane: PostgresControlPlane) =>
        controlPlane.completeStaleUploadReclaim(handle),
    },
    {
      ...
      complete: (controlPlane: PostgresControlPlane) =>
        controlPlane.completeHardDeleteQuarantined(handle),
    },
  ])(
    "keeps $name loud when the row remains in an unexpected state",
    async ({ complete, expectedError, results }) => {
      const fixture = scriptedControlPlane(results);

      await expect(complete(fixture.controlPlane)).rejects.toThrow(
        expectedError,
      );
  ...
  it("filters Path-1 references before LIMIT in both live selection and preview SQL", async () => {
    const first = brand<PreparedWriteHandle>(
      "00000000-0000-4000-8000-000000000011",
    );
    const second = brand<PreparedWriteHandle>(
    ...
    const selected = scriptedControlPlane([
      ...
      { rowCount: 1, rows: [{ count: 2 }] },
      {
        ...
        rows: [
          {
            prepared_blob_id: first,
            blob_path: `blobs/${first}`,
            blob_len: "1",
            state: "reclaim_marked",
            effective_reclaim_after_ms: null,
          },
        ],
      },
    ]);
    ...
    await expect(
      selected.controlPlane.selectFinalizedOrphansForReclaim({
        olderThanEpochMs: T0 + 1,
        limit: 2,
      }),
    ).resolves.toEqual({
      rows: [
        { preparedBlobId: first, blobPath: `blobs/${first}`, blobLength: 1n },
      ],
      skippedReferenced: 2,
    });
    const liveSelectorSql = selected.statements.find((statement) =>
      statement.includes("FOR UPDATE OF p SKIP LOCKED"),
    );
    ...
    const previewed = scriptedControlPlane([
      ...
      { rowCount: 1, rows: [{ count: 2 }] },
      {
        ...
        rows: [
          ...
          {
            prepared_blob_id: second,
            blob_path: `blobs/${second}`,
            blob_len: "1",
            state: "finalized",
            effective_reclaim_after_ms: null,
          },
    ...
    await expect(
      previewed.controlPlane.previewReclamation({
        olderThanEpochMs: T0 + 1,
    ...
    const previewSql = previewed.statements.find(
      (statement) =>
        statement.includes(
...
describe.skipIf(!LIVE)("PostgresControlPlane live conformance", () => {
  ...
  async function prepare(
    ...
    const retentionClass =
      expiresAtEpochMs === MATTER_EXPIRES_AT ? "matter" : "detector-only";
    const recordCreatedAtMs =
      options.createdAtMs ??
      (retentionClass === "detector-only"
        ? Number(expiresAtEpochMs - 86_400_000n)
        : T0);
    ...
    const lifecycleCreatedAtMs =
      options.lifecycleCreatedAtMs ?? options.createdAtMs;
  ...
  it("elects one DEK generation and reserves first=0 monotonic nonces concurrently without reuse", async () => {
    ...
    const [generationA, generationB] = await Promise.all([
      controlPlane.ensureDekGeneration({
        scope,
        mint: async () => ({
          dekGenerationId: brand<DekGenerationId>(
            "tenant-a\0matter-a\0reversal-v1\0gen-a",
          ),
          wrappedDek: brand<WrappedDekMaterial>(Uint8Array.of(1, 2, 3)),
        }),
      }),
      controlPlane.ensureDekGeneration({
        ...
        mint: async () => ({
          dekGenerationId: brand<DekGenerationId>(
            "tenant-a\0matter-a\0reversal-v1\0gen-b",
          ),
    ...
    expect(
      new Set([nonceValue(first), nonceValue(second), nonceValue(third)]),
    ).toEqual(new Set([0n, 1n, 2n]));
  });
  ...
  it("catalog-proves idempotent expand DDL with nullable columns and unvalidated constraints", async () => {
    ...
    expect(
      after.rows.some(
        (row) => row.conname === "reversal_prepared_gly345_required_check",
      ),
    ).toBe(false);
  ...
  it("pins one durable class per exact tenant/attempt and rolls a mismatch anchor back", async () => {
    ...
    await expect(
      new PostgresControlPlane(pool).insertPreparedUploading({
        ...
        idempotencyKey: brand<ReversalIdempotencyKey>(
          "binding-mismatch-second-idempotency",
        ),
        immutableScopeDigest: brand<ReversalScopeDigest>(
          "binding-mismatch-scope",
        ),
    ...
    expect(
      (await pool.query(`SELECT 1 FROM reversal_operation_retention`)).rowCount,
    ).toBe(1);
    expect((await pool.query(`SELECT 1 FROM reversal_prepared`)).rowCount).toBe(
      1,
    );
  });
  ...
  it("waits on speculative operation insertion and follows winner commit or rollback", async () => {
    const makeAnchor = (
      attempt: string,
      retentionClass: "matter" | "detector-only",
    ) => {
      const createdAtEpochMs = Date.now();
      return {
        preparedBlobId: brand<PreparedWriteHandle>(randomUUID()),
        tenantId: TENANT,
        mappingKey: brand<ReversalMappingKey>(
          `mapping-${attempt}-${retentionClass}`,
        ),
        idempotencyKey: brand<ReversalIdempotencyKey>(
          `idempotency-${attempt}-${retentionClass}`,
        ),
    ...
    for (const outcome of ["commit", "rollback"] as const) {
      ...
      expect(classes.rows).toEqual([
        { retention_class: outcome === "commit" ? "matter" : "detector-only" },
      ]);
    }
  });
  ...
  it("makes publish first-writer-wins across racing connections", async () => {
    ...
    const winner = [leftResult, rightResult].find(
      (result) => result.kind === "published",
    );
    const loser = [leftResult, rightResult].find(
      (result) => result.kind === "existing",
    );
    expect(winner?.kind).toBe("published");
    expect(loser?.kind).toBe("existing");
    if (winner?.kind !== "published" || loser?.kind !== "existing")
      throw new Error("missing race result");
    ...
    ).toEqual({ committed: "1", finalized: "1" });
  });
  ...
  it("flushes pending claims and CAS prevents an out-of-order lower ordinal rollback", async () => {
    ...
    expect(claims.rows).toEqual([
      ...
      {
        state: "flushed",
        prepared_blob_id: newer.prepared.handle as unknown as string,
      },
    ]);
    const oldPrepared = await pool.query<{
      readonly state: string;
      readonly superseded_at_ms: string | null;
    }>(
  ...
  it("tombstones expired pending, detaches and orphans its blob, and later retries stay expired", async () => {
    ...
    await pool.query(
      `UPDATE reversal_claim SET expires_at_ms = 0 WHERE commit_handle = $1`,
      [commit],
    );
    ...
    const claim = await pool.query<{
      ...
      `SELECT state, prepared_blob_id, expires_at_ms FROM reversal_claim WHERE commit_handle = $1`,
      [commit],
    );
  ...
  it("readCurrentPointers is exact-key, returns durable integrity attributes, and rejects a mismatched flush", async () => {
    ...
    const missing = brand<ReversalMappingKey>(
      `tenant-a${SEP}matter-a${SEP}1${SEP}[[Missing]]`,
    );
    const rows = await controlPlane.readCurrentPointers([
      missing,
      fixture.mappingKey,
      fixture.mappingKey,
    ]);
  ...
  it("Path-1 marks only old finalized/orphaned unreferenced rows and Path-3 uses quarantine time", async () => {
    ...
    const orphanWinner = await prepare({
      lifecycleCreatedAtMs: T0,
      attempt: "orphan",
      expiresAtMs: orphanExpiry,
    });
    ...
    const orphanLoser = await prepare({
      lifecycleCreatedAtMs: T0,
      attempt: "orphan",
      expiresAtMs: orphanExpiry,
    });
    ...
    const marked = await controlPlane.reclaimFinalizedOrphans({
      olderThanEpochMs: T0 + 50,
      limit: 20,
    });
    const ids = new Set(marked.map((row) => row.preparedBlobId));
    expect(ids).toEqual(
      new Set([
        finalized.prepared.handle,
        orphanWinner.prepared.handle,
        orphanLoser.prepared.handle,
      ]),
    );
    ...
    expect(
      await controlPlane.hardDeleteQuarantined({
        olderThanEpochMs: T0 + 200,
        limit: 10,
      }),
    ).toEqual([]);
    const hardDelete = await controlPlane.hardDeleteQuarantined({
      olderThanEpochMs: T0 + 201,
      limit: 10,
    });
    expect(hardDelete.map((row) => row.preparedBlobId)).toEqual([
      finalized.prepared.handle,
    ]);
  ...
  it("ORACLE-EXPAND-CONTRACT-OLD-WRITER admits NULL metadata and reclaims it after supersession", async () => {
    ...
    const oldIdempotency = encodeTextKey(
      `tenant-a${SEP}legacy-null-attempt${SEP}[[LegacyNull]]`,
    );
    await pool.query(
    ...
    await pool.query(
      `UPDATE reversal_prepared SET superseded_at_ms = 0 WHERE prepared_blob_id = $1`,
      [oldPrepared],
    );
    ...
    expect(selected.rows.map((row) => row.preparedBlobId)).toContain(
      oldPrepared,
    );
    const marked = await pool.query<{
      readonly state: string;
      readonly reclaim_after_ms: string;
    }>(
  ...
  it("EXPLAIN serves the superseded matter arm from its partial index", async () => {
    ...
    try {
      ...
      expect(JSON.stringify(explain.rows)).toContain(
        "reversal_prepared_superseded_reclaim_idx",
      );
    } finally {
  ...
  it("Path-2 reselects upload_reclaim_marked rows regardless of age after a simulated crash", async () => {
    ...
    await controlPlane.insertPreparedUploading({
      ...
      tenantId: TENANT,
      mappingKey: brand<ReversalMappingKey>(
        `tenant-a${SEP}matter-a${SEP}1${SEP}[[Upload]]`,
      ),
      idempotencyKey: brand<ReversalIdempotencyKey>(
        "tenant-a\0upload-attempt\0[[Upload]]",
      ),
    ...
    const marked = await controlPlane.reclaimStaleUploads({
  ...
  it("keeps a new uploading row fresh despite a client timestamp skewed 100 hours behind", async () => {
    ...
    await controlPlane.insertPreparedUploading({
      ...
      tenantId: TENANT,
      mappingKey: brand<ReversalMappingKey>(
        `tenant-a${SEP}matter-a${SEP}1${SEP}[[Skewed]]`,
      ),
      idempotencyKey: brand<ReversalIdempotencyKey>(
        "tenant-a\0skewed-attempt\0[[Skewed]]",
      ),
  ...
  it("stores NUMERIC(20,0) matter expiry exactly and compares without BIGINT overflow", async () => {
    ...
    expect(raw.rows[0]).toEqual({
      expires_at_ms: MATTER_EXPIRES_AT.toString(),
      pg_typeof: "numeric",
    });
    ...
    const boundary = await pool.query<{
      ...
      readonly before_boundary: boolean;
    }>(
      `SELECT
      ...
      [
        MATTER_EXPIRES_AT.toString(),
        (MATTER_EXPIRES_AT - 1n).toString(),
        commit,
      ],
    );
...
describe.skipIf(!LIVE)("GLY-345 live migration guards", () => {
  ...
  async function withBaseSchema<T>(
    label: string,
  ...
  async function seedLegacyClaim(
    ...
    const mapping = encodeTextKey(
      `tenant-a${SEP}matter-a${SEP}1${SEP}${options.token}`,
    );
    const idempotency = encodeTextKey(
      `tenant-a${SEP}${options.attempt}${SEP}${options.token}`,
    );
    await pool.query(
      `INSERT INTO reversal_prepared (
         prepared_blob_id, tenant_id, mapping_key, idempotency_key, scope_digest,
  ...
  it("aborts ambiguous finite legacy expiry atomically before schema mutation", async () => {
    await withBaseSchema("ambiguous_expiry", async (scoped) => {
      ...
      await expect(runMigrations(scoped)).rejects.toThrow(
        "gly345_ambiguous_legacy_expiry",
      );
      ...
      expect(
        (await scoped.query(`SELECT 1 FROM reversal_prepared`)).rowCount,
      ).toBe(1);
    });
  });
  ...
  it("aborts mixed legal legacy classes for one decoded operation atomically", async () => {
    await withBaseSchema("mixed_class", async (scoped) => {
      ...
      await expect(runMigrations(scoped)).rejects.toThrow(
        "gly345_mixed_legacy_operation_retention",
      );
      expect(
        (await scoped.query(`SELECT 1 FROM reversal_prepared`)).rowCount,
      ).toBe(2);
  ...
  it("drops arbitrary anonymous legacy state checks by definition", async () => {
    await withBaseSchema("anonymous_check", async (scoped) => {
      ...
      expect(
        surviving.rows.every((row) =>
          row.definition.toLowerCase().includes("superseded"),
        ),
      ).toBe(true);
    });
  });
  ...
  it("preserves every legacy lifecycle row and seeds only non-current flushed rows from migration DB now", async () => {
    await withBaseSchema("legacy_preservation", async (scoped) => {
      const mapping = encodeTextKey(
        `tenant-a${SEP}matter-a${SEP}1${SEP}[[LegacyCurrent]]`,
      );
      const oldPrepared = randomUUID();
      const currentPrepared = randomUUID();
      const oldCommit = randomUUID();
      const currentCommit = randomUUID();
      const oldId = encodeTextKey(
        `tenant-a${SEP}legacy-old${SEP}[[LegacyCurrent]]`,
      );
      const currentId = encodeTextKey(
        `tenant-a${SEP}legacy-current${SEP}[[LegacyCurrent]]`,
      );
      for (const [prepared, id] of [
        [oldPrepared, oldId],
        [currentPrepared, currentId],
      ] as const) {
        await scoped.query(
          `INSERT INTO reversal_prepared (
             prepared_blob_id, tenant_id, mapping_key, idempotency_key, scope_digest,
      ...
      for (const [index, row] of preserved.entries()) {
        const prepared = randomUUID();
        const idempotency = encodeTextKey(
          `tenant-a${SEP}preserved-${index}${SEP}[[T${index}]]`,
        );
        const rowMapping = encodeTextKey(
          `tenant-a${SEP}matter-a${SEP}1${SEP}[[T${index}]]`,
        );
        await scoped.query(
          `INSERT INTO reversal_prepared (
             prepared_blob_id, tenant_id, mapping_key, idempotency_key, scope_digest,
      ...
      expect(BigInt(old.rows[0]!.superseded_at_ms)).toBeGreaterThanOrEqual(
        BigInt(before.rows[0]!.db_now_ms),
      );
      expect(BigInt(old.rows[0]!.superseded_at_ms)).toBeLessThanOrEqual(
        BigInt(after.rows[0]!.db_now_ms),
      );
...
describe.skipIf(!LIVE || process.env.PHI_REVERSAL_PG_STRESS !== "1")(
  ...
  () => {
    ...
    it.each([
      ...
      "enforces the exact $rows row boundary",
      async ({ rows, succeeds }) => {
        ...
        try {
          ...
          if (succeeds) {
            ...
            await expect(runMigrations(scoped)).rejects.toThrow(
              "gly345_online_migration_row_limit_exceeded",
            );
            expect(
              (
                await scoped.query(
