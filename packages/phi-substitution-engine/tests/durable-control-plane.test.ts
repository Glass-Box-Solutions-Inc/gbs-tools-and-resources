...
function writeInput(
  ...
  const retentionClass =
    ...
    options.expiresAtMs === MATTER_EXPIRES_AT
      ? "matter"
      : "detector-only";
...
describe("GLY-346 Lane A — reclamation oracles", () => {
  it("previews reclamation with the global budget without mutating candidates", async () => {
    ...
    expect(controlPlane.debugPrepared(prepared.handle)?.state).toBe(
      "finalized",
    );
  ...
  it("keeps referenced rows outside the in-memory maintenance budget", async () => {
    ...
    c.set(T0 + 100);
    await controlPlane.reclaimOrphanedPrepared({
      olderThanEpochMs: T0 + 1,
      limit: 1,
    });
    c.advance(101);

    const first = await controlPlane.reclaimOrphanedPrepared({
      olderThanEpochMs: T0 + 1,
      limit: 2,
    });
    const second = await controlPlane.reclaimOrphanedPrepared({
      olderThanEpochMs: T0 + 1,
      limit: 2,
    });
  ...
  it("caps Path-1 selection when unreferenced candidates exceed the limit", async () => {
    const limit = 2;
    const selectionPlane = new InMemoryControlPlane({
      nowEpochMilliseconds: () => T0,
    });
    for (let index = 0; index < 5; index += 1) {
    ...
    const workerPlane = new InMemoryControlPlane({
      nowEpochMilliseconds: () => T0,
    });
    for (let index = 0; index < 5; index += 1) {
  ...
  it("MUT-RECLAIM-COMMITTED: a committed/current-referenced blob is never reclaimed", async () => {
    ...
    const read = await controlPlane.readCurrent([
  ...
  it("MUT-RECLAIM-PENDING-CLAIM: a blob backing a pending claim is not reclaimed", async () => {
    ...
    expect(controlPlane.debugClaim(input.idempotencyKey)?.state).toBe(
      "pending",
    );
    expect(controlPlane.debugClaim(input.idempotencyKey)?.preparedBlobId).toBe(
      prepared.handle,
    );
  });
  ...
  it("MUT-QUARANTINE-NOT-HARDDELETE: quarantine is recoverable within grace and deleted only past grace", async () => {
    ...
    await controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: T0 + 1 });
    expect(
      controlPlane.debugPrepared(prepared.handle)?.quarantineBlobPresent,
    ).toBe(true);
  ...
  it("reclamation arguments reject non-finite, negative, unsafe, and out-of-cap values", async () => {
    const controlPlane = new InMemoryControlPlane();
    await expect(
      controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: Number.NaN }),
    ).rejects.toThrow();
    await expect(
      controlPlane.reclaimOrphanedPrepared({
        olderThanEpochMs: Number.POSITIVE_INFINITY,
      }),
    ).rejects.toThrow();
    await expect(
      controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: -1 }),
    ).rejects.toThrow();
    await expect(
      controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: 0, limit: 0 }),
    ).rejects.toThrow();
    await expect(
      controlPlane.reclaimOrphanedPrepared({
        olderThanEpochMs: 0,
...
describe("GLY-346 Lane A — control-plane state-machine conformance", () => {
  it("uses the plane clock so stale producer metadata cannot age fresh Path-2a or Path-1 rows", async () => {
    ...
    const faults: InMemoryControlPlaneFaults = {
      failAt: "prepareAfterUploadingInsert",
    };
    const controlPlane = new InMemoryControlPlane({
      nowEpochMilliseconds: c.now,
    ...
    await expect(controlPlane.prepare(writeInput(skewedMeta))).rejects.toThrow(
    ...
    expect(outcome.reclaimed).toBe(0);
    expect(controlPlane.debugPrepared(uploadingHandle)?.state).toBe(
      "uploading",
    );
  ...
  it("uses an old plane clock so Path-2a and Path-1 become eligible despite fresh producer metadata", async () => {
    ...
    await expect(controlPlane.prepare(writeInput(freshMeta))).rejects.toThrow(
      "prepareAfterUploadingInsert",
    );
    const uploadingHandle = controlPlane.debugPreparedHandles()[0];
    expect(uploadingHandle).toBeDefined();
  ...
  it("orders Path-1 recovery and fresh candidates like Postgres, not insertion order", async () => {
    ...
    const freshLaterId = brand<PreparedWriteHandle>(
      "00000000-0000-4000-8000-000000000002",
    );
    const freshEarlierId = brand<PreparedWriteHandle>(
      "00000000-0000-4000-8000-000000000001",
    );
    const recoveryId = brand<PreparedWriteHandle>(
  ...
  it("publish-loss then crash preserves pending claim and retry recovers through flush", async () => {
    ...
    expect(
      await controlPlane.readCurrent([{ mappingKey: input.mappingKey }]),
    ).toHaveLength(1);
  });
  ...
  it("expired-pending tombstones and detaches, preserves expiry/key, orphans and remains expired", async () => {
    ...
    const winner = await prepare(controlPlane, {
    ...
    expect(controlPlane.debugPrepared(winner.prepared.handle)?.state).toBe(
      "orphaned",
    );
    const later = await controlPlane.publish(loser.prepared);
    expect(later).toMatchObject({ kind: "existing", expired: true });

    const reclaimed = await controlPlane.reclaimOrphanedPrepared({
      olderThanEpochMs: T0 + 1,
    });
  ...
  it("upload_reclaim_marked crash is recovered by the next age-independent sweep (N1)", async () => {
    ...
    await expect(
      controlPlane.prepare(writeInput({ createdAtMs: T0 })),
    ).rejects.toThrow("prepareAfterUploadingInsert");
    ...
    await expect(
      controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: T0 + 1 }),
    ).rejects.toThrow("reclaimAfterUploadMark");
    expect(controlPlane.debugPrepared(handle)?.state).toBe(
      "upload_reclaim_marked",
    );

    controlPlane.crash();
    delete faults.failAt;
    const recovered = await controlPlane.reclaimOrphanedPrepared({
      olderThanEpochMs: 0,
    });
  ...
  it("readCurrent throws when a durable current pointer has no blob", async () => {
    ...
    await expect(
      controlPlane.readCurrent([{ mappingKey: input.mappingKey }]),
  ...
  it("nonce first reservation is zero and subsequent values strictly increase without repeat across crash", async () => {
    ...
    expect([nonceValue(first), nonceValue(second), nonceValue(third)]).toEqual([
      0n,
      1n,
      2n,
    ]);
    expect(
      new Set([
        ...
        Buffer.from(third).toString("hex"),
      ]).size,
    ).toBe(3);
  });
  ...
  it("ordinal out-of-order flush does not roll the current pointer back", async () => {
    ...
    const older = await prepare(controlPlane, {
      attempt: "attempt-1",
      marker: 1,
    });
    const newer = await prepare(controlPlane, {
      attempt: "attempt-2",
      marker: 2,
    });
    const oldPublish = await controlPlane.publish(older.prepared);
    const newPublish = await controlPlane.publish(newer.prepared);
    if (oldPublish.kind !== "published" || newPublish.kind !== "published")
    ...
    expect(current?.ordinal).toBeGreaterThan(
      controlPlane.debugClaim(older.input.idempotencyKey)?.ordinal ?? 0n,
    );
    const read = await controlPlane.readCurrent([
      { mappingKey: older.input.mappingKey },
    ]);
  ...
  it("matter expiry exactly 2^64-1 stores and compares without overflow", async () => {
    ...
    const fixture = await prepare(controlPlane, {
      expiresAtMs: 2n ** 64n - 1n,
    });
    const published = await controlPlane.publish(fixture.prepared);
    expect(
      controlPlane.debugClaim(fixture.input.idempotencyKey)?.expiresAtMs,
    ).toBe(2n ** 64n - 1n);
    const retry = await controlPlane.publish(fixture.prepared);
    expect(retry).toMatchObject({ kind: "existing", expired: false });
    expect(controlPlane.debugClaim(fixture.input.idempotencyKey)?.state).toBe(
      "pending",
    );
    expect(published.kind).toBe("published");
  });
...
describe("GLY-345 Part B — superseded lifecycle conformance", () => {
  ...
  it("advances current and atomically supersedes/detaches the prior committed pair", async () => {
    ...
    const old = await prepare(controlPlane, {
      attempt: "supersede-old",
      marker: 1,
    });
    const oldCommit = await publishAndFlush(controlPlane, old);
    const fresh = await prepare(controlPlane, {
      attempt: "supersede-new",
      marker: 2,
    });
    const freshPublish = await controlPlane.publish(fresh.prepared);
    if (freshPublish.kind !== "published")
    ...
    expect(controlPlane.debugClaim(fresh.input.idempotencyKey)?.state).toBe(
      "flushed",
    );
    expect(controlPlane.debugPrepared(fresh.prepared.handle)?.state).toBe(
      "committed",
    );

    controlPlane.debugDeleteBlob(old.prepared.handle);
    await expect(controlPlane.flush(oldCommit)).resolves.toBeUndefined();
    expect(controlPlane.debugCurrent(old.input.mappingKey)?.commitHandle).toBe(
      freshPublish.commit,
    );
  });
  ...
  it("makes every intra-flush advance gate atomic and replay-recoverable", async () => {
    ...
    for (const phase of phases) {
      ...
      const old = await prepare(controlPlane, {
        attempt: `advance-old-${phase}`,
      });
      const oldCommit = await publishAndFlush(controlPlane, old);
      const fresh = await prepare(controlPlane, {
        attempt: `advance-new-${phase}`,
        marker: 2,
      });
      const freshPublish = await controlPlane.publish(fresh.prepared);
      if (freshPublish.kind !== "published")
      ...
      faults.failAt = phase;
      await expect(controlPlane.flush(freshPublish.commit)).rejects.toThrow(
        phase,
      );
      expect(
        controlPlane.debugCurrent(old.input.mappingKey)?.commitHandle,
        phase,
      ).toBe(oldCommit);
      expect(
        controlPlane.debugClaim(old.input.idempotencyKey),
        phase,
      ).toMatchObject({
        state: "flushed",
      ...
      expect(
        controlPlane.debugPrepared(old.prepared.handle)?.state,
        phase,
      ).toBe("committed");
      expect(
        controlPlane.debugClaim(fresh.input.idempotencyKey),
        phase,
      ).toMatchObject({
        state: "pending",
        preparedBlobId: fresh.prepared.handle,
      ...
      expect(
        controlPlane.debugPrepared(fresh.prepared.handle)?.state,
        phase,
      ).toBe("committed");
      ...
      expect(
        controlPlane.debugCurrent(old.input.mappingKey)?.commitHandle,
        phase,
      ).toBe(freshPublish.commit);
      expect(
        controlPlane.debugClaim(old.input.idempotencyKey)?.state,
        phase,
      ).toBe("superseded");
    }
  });
  ...
  it("makes both losing-CAS gates atomic and replay-recoverable", async () => {
    for (const phase of [
      ...
      const older = await prepare(controlPlane, {
        attempt: `loser-old-${phase}`,
        marker: 1,
      });
      const newer = await prepare(controlPlane, {
        attempt: `loser-new-${phase}`,
        marker: 2,
      });
      const oldPublish = await controlPlane.publish(older.prepared);
      const newPublish = await controlPlane.publish(newer.prepared);
      ...
      faults.failAt = phase;
      await expect(controlPlane.flush(oldPublish.commit)).rejects.toThrow(
        phase,
      );
      expect(
        controlPlane.debugCurrent(older.input.mappingKey)?.commitHandle,
        phase,
      ).toBe(newPublish.commit);
      expect(
        controlPlane.debugClaim(older.input.idempotencyKey),
        phase,
      ).toMatchObject({
        state: "pending",
      ...
      expect(
        controlPlane.debugPrepared(older.prepared.handle)?.state,
        phase,
      ).toBe("committed");
  ...
  it("computes matter and detector candidacy exactly and never selects live committed matter", async () => {
    ...
    const detector = await prepare(controlPlane, {
      attempt: "detector-old",
      expiresAtMs: detectorExpiry,
    });
    const detectorPublish = await controlPlane.publish(detector.prepared);
    if (detectorPublish.kind !== "published")
      throw new Error("expected publication");
    ...
    expect(
      controlPlane.debugPrepared(matter.prepared.handle)?.supersededAtMs,
    ).toBe(T0);
    expect(
      controlPlane.debugPrepared(detector.prepared.handle)?.supersededAtMs,
    ).toBe(T0);
    c.set(T0 + 39);
    await expect(
    ...
    expect(detectorDue.rows.map((row) => row.preparedBlobId)).toEqual([
      detector.prepared.handle,
    ]);
    expect(
      controlPlane.debugPrepared(detector.prepared.handle)?.reclaimAfterMs,
    ).toBe(BigInt(T0 + 40));
    ...
    expect(matterDue.rows.map((row) => row.preparedBlobId)).toEqual([
      matter.prepared.handle,
    ]);
    expect(matterDue.skippedReferenced).toBe(0);
    expect(
      controlPlane.debugPrepared(matter.prepared.handle)?.reclaimAfterMs,
    ).toBe(BigInt(T0 + 100));
    expect(controlPlane.debugPrepared(winner.prepared.handle)?.state).toBe(
      "committed",
    );
  ...
  it("keeps the drain MAX floor and ignores backfilled record-created time", async () => {
    ...
    const winner = await prepare(controlPlane, {
      attempt: "drain-floor-winner",
    });
    await publishAndFlush(controlPlane, winner);
    controlPlane.debugSetRecordRetentionOrigin(
      detector.prepared.handle,
      0,
      "backfilled",
    );
    ...
    expect(
      ...
      ).rows.map((row) => row.preparedBlobId),
    ).toEqual([detector.prepared.handle]);
  });
  ...
  it("keeps the detector floor inert at production windows because authenticated expiry wins", async () => {
    ...
    await publishAndFlush(controlPlane, detector);
    const winner = await prepare(controlPlane, {
      attempt: "production-detector-winner",
    });
    await publishAndFlush(controlPlane, winner);
  ...
  it.each([
    ...
    "stacks $name candidacy, quarantine grace, and later full-sweep latency",
    async ({ name, expiresAtMs, candidacyDelta }) => {
      const c = clock(T0);
      const controlPlane = new InMemoryControlPlane({
        nowEpochMilliseconds: c.now,
      ...
      const old = await prepare(controlPlane, {
        attempt: `stack-old-${name}`,
        expiresAtMs,
      });
      await publishAndFlush(controlPlane, old);
      const winner = await prepare(controlPlane, {
        attempt: `stack-winner-${name}`,
      });
      await publishAndFlush(controlPlane, winner);
      ...
      await controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: 0 });
      expect(controlPlane.debugPrepared(old.prepared.handle)?.state).toBe(
        "quarantined",
      );
  ...
  it("keeps a durable operation binding after every prepared byte/row is reclaimed", async () => {
    ...
    const anchored = await prepare(controlPlane, {
    ...
    await controlPlane.reclaimOrphanedPrepared({ olderThanEpochMs: T0 + 1 });
    expect(
      controlPlane.debugPrepared(anchored.prepared.handle),
    ).toBeUndefined();
    expect(
      controlPlane.debugOperationRetention(
        String(TENANT),
        "binding-survives-gc",
      ),
    ).toBe("matter");
  ...
  it("counts but never marks a referenced synthetic superseded row", async () => {
    ...
    const fixture = await prepare(controlPlane, {
      attempt: "referenced-superseded",
    });
    const published = await controlPlane.publish(fixture.prepared);
    if (published.kind !== "published") throw new Error("expected publication");
    controlPlane.debugSetSupersededMetadata(
    ...
    expect(controlPlane.debugPrepared(fixture.prepared.handle)?.state).toBe(
      "superseded",
    );
  });
  ...
  it("self-heals a stale non-current flushed pair under both crash gates, even without its blob", async () => {
    for (const phase of [
      ...
      const old = await prepare(controlPlane, {
        attempt: `self-heal-old-${phase}`,
      });
      const oldCommit = await publishAndFlush(controlPlane, old);
      const fresh = await prepare(controlPlane, {
        attempt: `self-heal-new-${phase}`,
        marker: 2,
      });
      ...
      controlPlane.debugSetClaimLifecycle(
        old.input.idempotencyKey,
        "flushed",
        old.prepared.handle,
      );
      controlPlane.debugDeleteBlob(old.prepared.handle);
      ...
      faults.failAt = phase;
      await expect(
        Promise.resolve().then(() => controlPlane.publish(old.prepared)),
    ...
    async (phase) => {
      ...
      await expect(
        Promise.resolve().then(() =>
          controlPlane.prepare(writeInput({ attempt: `anchor-${phase}` })),
        ),
      ).rejects.toThrow(phase);
      expect(
        controlPlane.debugOperationRetention(String(TENANT), `anchor-${phase}`),
      ).toBeUndefined();
