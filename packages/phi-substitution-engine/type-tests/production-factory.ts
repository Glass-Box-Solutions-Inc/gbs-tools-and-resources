...
const generated: Promise<ProtectedAiTextResult> = surface.generateText({
  prompt: "x",
});
void generated;

// R2: optional widening preserves callers that do not supply the legacy fallback capability.
declare const withoutInvokeRaw: Omit<
  ProtectedAiProviderDependencies<{ prompt: string }, unknown>,
  "invokeRaw"
>;
const widenedDependencies: ProtectedAiProviderDependencies<
  { prompt: string },
  unknown
> = withoutInvokeRaw;
void widenedDependencies;

declare const productionDependencies: CreateProductionProtectedAiProviderOptions<{
  prompt: string;
}>;
...
const originalEgressAuthorizer =
  createProductionProtectedOriginalEgressAuthorizer(originalEgressOptions);
...
declare const missingEgressVersion: Omit<
  AzureEgressPolicyEvidence,
  "egressPolicyVersion"
>;
// @ts-expect-error egressPolicyVersion is a required signed claim.
const invalidMissingEgressVersion: AzureEgressPolicyEvidence =
  missingEgressVersion;
void invalidMissingEgressVersion;

declare const missingEngineVersion: Omit<
  AzureEgressPolicyEvidence,
  "enginePolicyVersion"
>;
// @ts-expect-error enginePolicyVersion is a required signed claim.
const invalidMissingEngineVersion: AzureEgressPolicyEvidence =
