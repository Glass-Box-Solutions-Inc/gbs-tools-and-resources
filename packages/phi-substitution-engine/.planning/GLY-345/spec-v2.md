...
# GLY-345 specification v2 — durable operation-retention binding and supersede-aware retention
...
3. This v2 supersedes GLY-345 spec-v1 and resolves Opus findings F1–F11 under the orchestrator's rulings.
...
`full` maintenance mode. Its Path-1 eligibility deadline is:

1. matter: `superseded_at_ms + SUPERSEDE_RETENTION_MS`; and
2. detector-only: `MIN(retention_expires_at_ms, superseded_at_ms + SUPERSEDE_RETENTION_MS)`.

That is the retention deadline. Physical rename additionally waits for the independent `READ_DRAIN_MS` heuristic;
quarantine fallback/grace—not the heuristic—carries read correctness.
...
- `reclaim_after_ms` is a nullable materialized-at-mark audit value. For a superseded row the maintenance
  selector computes it from its configured `SUPERSEDE_RETENTION_MS`: matter uses
  `superseded_at_ms + SUPERSEDE_RETENTION_MS`; detector-only uses the lesser of that deadline and
  `retention_expires_at_ms`. The transition to `reclaim_marked` writes the computed value atomically. There is no
  MaxUint64 exclusion for superseded matter. Physical marking also waits until the independent
  `superseded_at_ms + READ_DRAIN_MS` rename-delay heuristic; that floor is not stored as retention.
...
non-negative values. The current 24-hour grace default satisfies the second inequality; the 30-day default
satisfies the first.
...
there is no unbounded validation scan. The delta deletes no row/blob. Running it twice MUST converge. Malformed
keys, ambiguous legacy expiry, mixed historical class, or broken pointer invariants roll back the whole migration.

```sql
-- GLY-345 v2 additive delta. runMigrations() already wraps this file in one transaction.
...
-- Named v2 constraints/FK may exist on a rerun. Recreate them NOT VALID; do not validate here.
...
ALTER TABLE reversal_prepared DROP CONSTRAINT IF EXISTS reversal_prepared_gly345_required_check;
...
-- Replica-written flushed_at_ms is not trusted as a future lifecycle timestamp. Clamp it into
-- [0, migration DB now]. reclaim_after_ms remains NULL until maintenance materializes its configured deadline.
WITH migration_clock AS (
  SELECT (extract(epoch FROM clock_timestamp()) * 1000)::bigint AS db_now_ms
), legacy_superseded AS (
  SELECT p.prepared_blob_id,
         least(greatest(cur.flushed_at_ms, 0), mc.db_now_ms) AS clamped_superseded_at_ms
  FROM reversal_claim c
...
SET state = 'superseded',
    superseded_at_ms = s.clamped_superseded_at_ms,
...
ALTER TABLE reversal_prepared
  ADD CONSTRAINT reversal_prepared_gly345_required_check CHECK (
    operation_key IS NOT NULL
    AND record_created_at_ms IS NOT NULL
    AND retention_class IS NOT NULL
    AND retention_origin IS NOT NULL
...
effective reclaim deadline from that DB-clock supersession time and its configured policy window.
...
- `claim.state='flushed'` but current is a different commit of equal/higher ordinal: whether discovered by the
  `publish` conflict branch or direct `flush`, self-heal the stale pair in place. Under the mapping mutex, require
  the old prepared row is `committed` and no other claim/current references it. Execute this **single
...
| `committed` | higher ordinal advances current | old claim/pointer/prepared exact-match; all row counts one | `superseded` | same PG tx as new flushed claim and pointer CAS; DB-clock superseded time; deadline not yet materialized | fresh Path-1 at computed supersede deadline |
| `committed` | own ordinal loses current CAS | subject claim pending; current ordinal is greater/equal | `superseded` | claim→superseded/null + prepared transition in same PG tx; DB-clock superseded time | fresh Path-1 at computed supersede deadline |
| `committed` | stale non-current flushed claim discovered | different current has equal/higher ordinal; no other references | `superseded` | §6.2 one-statement claim-detach+prepared-supersede CTE, then commit | fresh Path-1 at computed supersede deadline |
| `orphaned` | Path-1 fresh claim | ordinary horizon passed; no references | `reclaim_marked` | PG tx | Path-1 recovery |
| `superseded` | Path-1 fresh claim | policy deadline from class/window and DB-clock rename-delay floor are both due; no references | `reclaim_marked` | PG tx atomically materializes policy `reclaim_after_ms` and marks | Path-1 recovery |
...
maintenance option. PostgreSQL captures its own `db_now_ms = clock_timestamp()` once in the selector transaction;
the job/replica clock is not authoritative. Define `effective_reclaim_after_ms` without consulting
...
OR (
  p.state = 'superseded'
  AND CASE p.retention_class
    WHEN 'detector-only' THEN least(
      p.retention_expires_at_ms,
      p.superseded_at_ms::numeric + $supersedeRetentionMs::numeric
    )
    ELSE p.superseded_at_ms::numeric + $supersedeRetentionMs::numeric
  END <= $dbNowMs::numeric
  AND p.superseded_at_ms::numeric + $readDrainMs::numeric <= $dbNowMs::numeric
)
```
...
`reclaim_marked`; for each superseded winner the same UPDATE materializes that CASE result into
`reclaim_after_ms`. Preview uses the identical expression/order but no locks/mutations. There is no MaxUint64
special case in this superseded arm. The second predicate is only a DB-clock rename-delay heuristic; it does not
create a reader lease or alter the materialized retention deadline. Referenced candidates retain the existing
...
2. `READ_DRAIN_MS` delays rename only as a heuristic. The stronger configured relation
   ...
   that delay; the separate selector floor also delays a detector whose policy deadline is earlier. Correctness
...
| before F11 self-heal CTE | stale flushed/non-current pair unchanged | retry re-enters self-heal |
...
| superseded row before effective supersede deadline | row remains superseded with null materialized deadline | no selector may mark it |
...
5. Matter policy deadline is exactly `superseded_at + SUPERSEDE_RETENTION_MS`, including MaxUint64 records.
   Detector-only policy deadline is the exact earlier of its numeric 24-hour expiry and that window. Selection is
   inclusive once that policy deadline **and** the separate DB-clock rename-delay heuristic are due. A
   non-superseded committed MaxUint64 matter row is never selected because committed is not a candidate.
...
12. Seed an unexpired/matter flushed claim whose mapping current is a different higher/equal ordinal. Publish
    replay performs the §6.2 one-statement self-heal, returns success through the superseded no-op flush, detaches
    the claim, supersedes the prepared row, and does not move current; repeat with the stale blob absent. An
    expired-detector replay performs the same heal before preserving its non-retryable error. Fault before commit
...
1. Apply 0001+delta to an empty DB twice and compare catalog definitions/counts; all added hot-table CHECK/FK
   constraints remain `convalidated=false`, added columns remain nullable, and no hot-table unique index exists.
...
9. Write a legacy future `flushed_at_ms`; migration clamps `superseded_at_ms` to migration DB now. Write a sane
   past value; it is preserved. Runtime supersession likewise uses `clock_timestamp()`, not supplied replica now.
...
12. `EXPLAIN` confirms operation PK, mapping mutex PK, prepared superseded partial index, and reference indexes are
...
- `MUT-MIGRATION-TRUST-FUTURE-FLUSHED-AT`
...
5. Superseded matter policy uses `superseded_at + SUPERSEDE_RETENTION_MS`, including MaxUint64 records;
   detector-only uses the earlier original-expiry/window deadline. Physical selection additionally honors the
   DB-clock rename-delay heuristic. Non-superseded committed matter remains outside every selector.
...
8. The additive migration succeeds twice on empty and bounded populated databases, leaves new hot-table
   constraints NOT VALID/columns nullable, and fails atomically on row-count, expiry, class, key, or pointer
   ambiguity.
...
8. No `VALIDATE CONSTRAINT`, `SET NOT NULL`, or hot-table unique-index build in this online migration. A follow-up
   validation migration is explicitly out of scope and needs its own rollout/window.
...
None for the principal. This v2 adopts the binding rulings: superseded matter gets a configurable 30-day default
window (detector uses the earlier deadline); the online migration is nullable/NOT VALID with a 100,000-row guard;
and first-seen bindings have no in-band override. `READ_DRAIN_MS` is only a rename heuristic, DB time is
authoritative, and quarantine fallback+grace carry read correctness.
...
| F5 | DB `clock_timestamp()` is authoritative; legacy future timestamps are clamped: migration SQL §4.2, runtime §6.1, selector §8.1, oracle §11.3.9. |
