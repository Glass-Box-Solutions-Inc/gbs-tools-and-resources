...
# already printed. A caller that knows what the evaluator concluded had no way
# to say so.
#
# The seam is opt-in and additive. A caller puts an ``apportionment`` and/or a
# ``causation`` block on ``doc_spec.context`` — the same channel the template
# already reads ``variant`` from — and this template renders that content.
# Absent, the substrate's own path runs untouched, which is the hard
# requirement: ``wc-synthetic-caseload-engine`` consumes this package as a
# library and its golden corpora pin every generated byte.
#
#   context["apportionment"] = {
#       "register": "none" | "apportioned" | "deferred",
#       "nonindustrial_pct": 30,          # the share apportioned away
#       "basis": "pre-existing degenerative disc disease",
#       "opinion": "...",                 # verbatim; outranks register
#   }
#   context["causation"] = {
#       "attribution": "predominantly",   # the adverb only
#       "discussion": "...",              # verbatim; outranks attribution
#   }
#
# Nothing here derives, infers or invents a value: the seam accepts what it is
# given. Deciding *what* a QME should opine belongs to the caller's ledger.
# --------------------------------------------------------------------------
...
_APPORTIONMENT_DEFAULT_BASIS = "nonindustrial factors"


def _seam_block(doc_spec, key):
    """The caller's governance block for ``key``, or ``None`` when ungoverned.

    Missing, ``None``, empty, and not-a-mapping all mean the same thing. The
    consuming engine sets a dozen keys on every context and expects a template
    that has not opted in to ignore them, so a block arriving empty from a
    caller that governs nothing has to read exactly like no block at all —
    anything else half-renders a document on a caller's silence.
    """
...
def _governed_apportionment_pct(doc_spec):
    """The ledger's nonindustrial share, or ``None`` to let the substrate draw."""
    block = _seam_block(doc_spec, "apportionment")
    if block is None or block.get("nonindustrial_pct") is None:
        return None
    return int(block["nonindustrial_pct"])


def _apportionment_opinion(block, drew_no_apportionment):
    """The apportionment sentence, governed or drawn.

    Precedence is explicit because two callers writing the same document must
    get the same words: an explicit ``opinion`` wins, then ``register``, then a
    register inferred from the percentage alone — M3 pins numbers and should not
    have to also pin the word for the number. ``drew_no_apportionment`` carries
    the coin the substrate already flipped, so the ungoverned answer is the
    substrate's own.
    """
    if block is None:
        return _APPORTIONMENT_NONE if drew_no_apportionment else _APPORTIONMENT_ADDRESSED

    opinion = block.get("opinion")
    if opinion:
        return str(opinion)
    ...
    if register is None:
        if pct is None:
            # A block naming neither an opinion, a register nor a number governs
            # nothing. Fall back rather than invent an opinion for the evaluator.
            return _APPORTIONMENT_NONE if drew_no_apportionment else _APPORTIONMENT_ADDRESSED
        register = "apportioned" if int(pct) > 0 else "none"

    if register == "deferred":
        return _APPORTIONMENT_DEFERRED
    if register == "none":
        return _APPORTIONMENT_NONE

    nonindustrial = int(pct) if pct is not None else 0
    basis = block.get("basis") or _APPORTIONMENT_DEFAULT_BASIS
    return (
        f"{100 - nonindustrial}% of the permanent disability is attributable to the "
        f"industrial injury and {nonindustrial}% is apportioned to {basis}, per LC §4663 "
        f"and §4664. This apportionment rests on the history, the records reviewed, and the "
        f"clinical findings documented in this report."
    )
...
class QmeAmeReport(BaseTemplate):
    ...
    def build_story(self, doc_spec):
        ...
        # Governed by the same block the conclusions read, so the percentage
        # rated here and the opinion stated in section 9 cannot disagree.
        story.extend(self.impairment_rating_section(
            apportionment_pct=_governed_apportionment_pct(doc_spec),
        ))
    ...
    def _build_conclusions(self, injury, doc_spec):
        ...
        causation = _seam_block(doc_spec, "causation")
        apportionment = _seam_block(doc_spec, "apportionment")

        if causation and causation.get("discussion"):
            causation_text = str(causation["discussion"])
        else:
            attribution = (causation or {}).get("attribution") or drawn_attribution
            causation_text = (
        ...
        apportionment_text = _apportionment_opinion(apportionment, drew_no_apportionment)
