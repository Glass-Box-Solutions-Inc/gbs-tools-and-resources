...
- [ ] ISC-85: `case_facts.py` defines a `CaseFacts` ledger (diagnostics performed/absent with modality+body part+date, surgery state + chosen CPT, provider list, treatment visit series, MMI date, WPI/PD figures) derived at plan time; double-derivation from the same seed is identical (probe: model dump compare)
- [ ] ISC-86: `scenario.diagnostics {performed, absent}` accepted in `CaseSeed`; loader rejects performed/absent overlap and unknown modalities with actionable errors (probe: bad-seed pytest)
- [ ] ISC-87: A seed with no `scenario` block auto-derives CaseFacts deterministically from the seed RNG (probe: double-run identity without scenario)
- [ ] ISC-88: Per-case manifest carries a `caseFacts` block (diagnostics, surgery, providers) (probe: manifest pytest)
- [ ] ISC-89: Renderer gains a fact-aware override registry (subtype → engine template subclass); subtypes not in the registry take the exact original code path (probe: dispatch assertion pytest)
- [ ] ISC-90: `DIAGNOSTICS_IMAGING` rendered content names the ledger's modality/body part for that document, never a private draw (probe: text extraction per performed diagnostic)
- [ ] ISC-91: QME/AME report imaging citations name only ledger-performed diagnostics; a ledger-absent modality never appears (probe: extraction grep + positive control seeding `absent: [emg]`)
- [ ] ISC-92: `scenario.surgery: performed` → operative documents emitted and PR/QME text references the surgery; `none` → zero surgical language anywhere in the case (anti-probe with positive control)
- [ ] ISC-93: The chosen surgical CPT is recorded in CaseFacts and named consistently in the operative record and every referencing document (probe: cross-document grep)
- [ ] ISC-94: Subpoenaed-records provider attribution round-robins over `CaseFacts.providers` (restores the substrate `provider_index` behavior the engine path dropped) (probe: multi-packet case, distinct providers in extracted text)
- [ ] ISC-95: A coherence harness sweeps ledger-vs-extracted-text for every rule above with ≥1 positive control per rule (probe: harness pytest, controls fail red when planted)
- [ ] ISC-96: Anti: a case containing no override-registry subtypes and no `scenario` block renders byte-identical to v0.2.0 (probe: targeted md5 compare)
- [ ] ISC-97: Demo double-run + `TZ=Australia/Sydney` identity holds with the facts layer active (probe: tree md5 ×3)
- [ ] ISC-98: Version 0.3.0 with numbered compatibility notices for byte changes on override-covered subtypes (probe: `--version` + CHANGELOG grep)
- [ ] ISC-99: Resolved CaseFacts surfaced per case as `case_facts.yaml` beside the resolved seed (probe: output-dir listing + load round-trip)
- [ ] ISC-100: Full suite + ruff green; no file under `merus-test-data-generator/` modified (probe: suite run + git diff empty on substrate)
