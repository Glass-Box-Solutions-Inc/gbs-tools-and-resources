...
class MerusCaseAPIClient:
    ...
    async def add_party(self, party: Party) -> APIResponse:
        ...
        # Fix: CakePHP wrapper required for parties/add
    ...
    async def add_activity(self, activity: Activity) -> APIResponse:
        ...
        # Fix: CakePHP wrapper required for activities/add
    ...
    async def list_documents(
        ...
        """
        List uploaded documents via GET /uploads/index (confirmed working, 9,419+ files).

        Args:
            case_file_id: Filter by case ID
            limit: Max results
        ...
        if case_file_id:
            params["case_file_id"] = case_file_id
        return await self._request("GET", "/uploads/index", params=params)

    async def download_document(self, upload_id: int) -> APIResponse:
        """
        Download document via GET /documents/download/{upload_id} → 302 → S3 signed URL.

        Returns file bytes in .data on success.

        Args:
            upload_id: Document upload ID

        Returns:
            APIResponse with file bytes in .data on success
        """
        try:
            response = await self._client.request(
                "GET",
            ...
            if response.status_code == 302:
                ...
                if not s3_url:
                    return APIResponse(success=False, error="Redirect missing Location header")
                async with httpx.AsyncClient(timeout=60) as s3:
                    r = await s3.get(s3_url)
                    if r.status_code == 200:
                        return APIResponse(success=True, data=r.content)
                    return APIResponse(
                        success=False,
                        error=f"S3 fetch failed: {r.status_code}",
                        error_code=r.status_code,
                    )
            elif response.status_code == 404:
                return APIResponse(
                    success=False,
                    error=f"Document {upload_id} not found",
                    error_code=404,
                )
            else:
                return APIResponse(
            ...
            return APIResponse(success=False, error="Download timed out", error_code=408)
        except Exception as e:
    ...
    async def get_firm_users(self) -> APIResponse:
        ...
        """
        # Fix: correct endpoint is /users/index (not /firmUsers/index)
        return await self._request("GET", "/users/index")
