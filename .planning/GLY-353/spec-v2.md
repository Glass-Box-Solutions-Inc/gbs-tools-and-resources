...
# GLY-353 specification v2 — production protected-AI composition factory
...
3. The caller-facing promise rejects with a fresh fixed `PhiEngineError("CALL_INTERRUPTED")`; `CALL_INTERRUPTED` is additive to the fixed failure-code union/error allow-list, but the interruption audit event has `failureCode: null` because interruption is not a failure terminal. No abort reason crosses any boundary.
4. If the signal is already aborted on entry, the wrapper obtains trusted context so the attempt can be durably represented, emits PREPARE, finalizes `interrupted`, and invokes route/project/substitution/provider zero times. If trusted context itself cannot be obtained, the existing missing-context failure remains authoritative because no trusted attempt identity exists to finalize.
...
3. Legacy `AiProvider`, `RawProviderPort`, `DevBoundaryProvider`, and `ProtectedStreamResult` remain unchanged. The new production port/result types do not replace or widen them.
...
- `src/audit/ports.ts`, serializer, and exact allow-list tests: additive `interrupted` audit outcome with `failureCode: null`.
